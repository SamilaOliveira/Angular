...
export class ConversorService {
  ...
  converter(conversao: Conversao): Observable<any> {
    let params = `?base=${conversao.moedaDe}-${conversao.moedaPara}`;
   ...
   * @return number
   */
  cotacaoPara(conversaoResponse: ConversaoResponse, conversao: Conversao):
  number {
   ...
   * @return string
  */
 cotacaoDe(conversaoResponse: ConversaoResponse, conversao: Conversao):
 string {
   if (conversaoResponse === undefined || conversao.moedaPara === undefined){
     return '0';
   }
  ...
  * @return string
  */
 dateCotacao(conversaoResponse: ConversaoResponse): string {
